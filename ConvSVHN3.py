...
test_labels = np.zeros((((testdata['y'])[:, 0]).size, ((testdata['y'])[:, 0]).max()))
test_labels[np.arange(((testdata['y'])[:, 0]).size), ((testdata['y'])[:, 0]) - 1] = 1
print("Test Label Shape: " + str(test_labels.shape))

train_images = traindata['X']
# train_labels = tf.one_hot((traindata['y'])[:, 0], 10)
train_labels = np.zeros((((traindata['y'])[:, 0]).size, ((traindata['y'])[:, 0]).max()))
train_labels[np.arange(((traindata['y'])[:, 0]).size), ((traindata['y'])[:, 0]) - 1] = 1
print("Test Label Shape: " + str(train_labels.shape))
...
def main():
    ...
    # next_train = train_labels.eval()
    # next_test = test_labels.eaval()

    file_writer = tf.summary.FileWriter(logdir, sess.graph)

    for i in range(num_of_batches):
        idx = np.random.choice(np.arange(len(train_labels[:, 0])), 100, replace=False)
        # temp_lbl = sess.run(next_train)
        batch_X = train_inputs[idx]
        batch_Y = train_labels[idx]

        # Run training analytics every so many batches
        if i%25 == 0:
            a, c, w_str, b_str, ca, da, l = sess.run([acc_summary_train,
                                loss_summary_train,
                                weights_summary,
                                biases_summary,
                                conv_act_summary,
        ...
        if i%100 == 0:
            # temp_lbl = sess.run(next_test)
            idx = np.random.choice(np.arange(len(test_labels[:, 0])), 100, replace=False)
            # temp_lbl = sess.run(next_train)
